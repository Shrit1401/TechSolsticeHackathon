...
import { metricState } from '@/lib/graphUtils'
...
export function widgetMeta(id: WidgetId): { title: string; subtitle?: string } {
  const m: Record<WidgetId, { title: string; subtitle?: string }> = {
    ...
    throughput: { title: 'Throughput & error budget', subtitle: 'Request rate with error overlay' },
    ...
    'service-map': { title: 'Service dependency map', subtitle: 'Topology (simplified)' },
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col justify-between gap-2">
      <p className="font-numeric-dial text-4xl text-[var(--accent-cyan)]">
        <CountUp value={latest} /> <span className="text-lg font-medium text-[var(--text-tertiary)]">req/s</span>
      </p>
...
function ErrorRateBody() {
  ...
  const latest = series[series.length - 1]?.value ?? 0
  const st = metricState('errorRate', latest)
  return (
    <div className="flex min-h-0 flex-1 flex-col justify-between gap-2">
      <div>
        <div className="mb-2">
          <StatusBadge state={st === 'healthy' ? 'healthy' : st === 'elevated' ? 'elevated' : 'critical'} />
        </div>
        <p className="font-numeric-dial text-4xl text-[var(--accent-cyan)]">
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col justify-between gap-2">
      <p className="font-numeric-dial text-4xl text-[var(--text-primary)]">
        <CountUp value={latest} /> <span className="text-lg font-medium text-[var(--text-tertiary)]">ms</span>
      </p>
...
function DerivedSparkBody({
  ...
  const series = useMemo(
    () => deriveSparkSeries(template, seed, base, amplitude, { min: clampMin, max: clampMax }),
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col justify-between gap-2">
      <p className={cn('font-numeric-dial text-4xl', textClass)}>
        <CountUp value={latest} decimals={decimals} />{' '}
...
function CpuBody() {
  ...
  useEffect(() => {
    const id = setInterval(() => setV(gaugeValue(Date.now(), 'cpu')), 2000)
...
function MemoryBody() {
  ...
  useEffect(() => {
    const id = setInterval(() => setV(gaugeValue(Date.now() + 99, 'mem')), 2000)
    return () => clearInterval(id)
  }, [])
            ...
            content={({ payload }) =>
              payload?.length ? (
                <div className="font-numeric-dial rounded-lg border border-white/[0.1] bg-black px-2 py-1 text-[11px] text-[var(--text-secondary)]">
                  HTTP/1.1 {Math.round(Number(payload[0]?.value))} · HTTP/2 {Math.round(Number(payload[1]?.value))} · WS{' '}
                  {Math.round(Number(payload[2]?.value))}
                </div>
              ) : null
            }
          />
          <Area type="natural" dataKey="h1" stackId="a" stroke="#4d7cfe" fill={`url(#c1-${uid})`} />
          <Area type="natural" dataKey="h2" stackId="a" stroke="#00e5ff" fill={`url(#c2-${uid})`} />
          <Area type="natural" dataKey="ws" stackId="a" stroke="#ffb020" fill={`url(#c3-${uid})`} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  )
    ...
    <div className="flex min-h-0 flex-1 flex-col justify-between gap-3">
      <p
        className={cn(
          ...
          score > 0.7 ? 'text-[var(--accent-red)] drop-shadow-[0_0_12px_rgba(255,77,106,0.45)]' : 'text-[var(--text-primary)]'
        )}
      >
        {score.toFixed(2)}
      </p>
...
function ServiceMapBody() {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col justify-center overflow-x-auto overflow-y-hidden pb-0.5">
          ...
          return (
            <Fragment key={s.id}>
              ...
              >
                ...
                <p className={cn(labelClass, 'line-clamp-2')} title={s.displayName}>
                  {s.displayName}
                </p>
              </div>
            </Fragment>
...
function IncidentTimelineBody() {
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col justify-center pt-2">
      <div
        className={cn(
      ...
      >
        {events.length > 0 && <div className="absolute left-0 right-0 top-1/2 h-px bg-white/15" />}
        {events.length === 0 ? (
